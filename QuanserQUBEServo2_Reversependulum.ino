...
*/
float theta_Kp = -6.7082;
float theta_Kd = -2.3141;
float alpha_Kp = 44.5;
float alpha_Kd = 3.8753;
float motorVoltage;
...
//Include here your code
    motorVoltage = -(theta_n*theta_Kp)-(theta_dot*theta_Kd)-(alpha_n*alpha_Kp)-(alpha_dot*alpha_Kd);
      // set the saturation limit to +/- 10V     
    if(abs(motorVoltage)>18){
       motorVoltage=(motorVoltage/abs(motorVoltage))*18;
    }
    if(abs(alpha)>=.17453*2||abs(theta)>=3.14/2){
       motorVoltage=0;
    }
//Include here your code
    theta_n_k1 = theta_n; 
